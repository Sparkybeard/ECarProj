 ...
 * Park
 *
 * A car park is defined by an ID, a set of Coordinates, capacity and retrieval
 * bonus.
 *
 */
public class Park {
	...
	/** Maximum capacity of park. */
	private int maxCapacity;
	/** Bonus for returning vehicle at this park. */
	private int bonus;
	...
	/** Synchronized locks object before attempting to return car */
	public synchronized int returnCar() throws NoSlotAvailException {
		if (getFreeDocks() == 0)
	...
	/** Synchronized locks object before attempting to get car */
	public synchronized void getCar() throws NoCarAvailException {
		if (getFreeDocks() == getMaxCapacity())
			throw new NoCarAvailException();
	...
	/** Synchronized locks object before returning available vehicles */
	public synchronized int getAvailableCars() {
